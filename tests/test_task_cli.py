...
def test_task_start_with_pending_task(mock_core_class):
    ...
    deliverable1 = Deliverable(
        name="Deliverable 1", slug="deliverable-1", actions=[action1]
    )
    objective1 = Objective(
        name="Objective 1", slug="objective-1", deliverables=[deliverable1]
    )
    milestone1 = Milestone(
        name="Milestone 1", slug="milestone-1", objectives=[objective1]
    )
...
def test_task_done_with_in_progress_task(mock_core_class):
    ...
    current_action = Action(
        name="Current Task", slug="current-task", status="in-progress"
    )
...
def test_task_next_with_in_progress_task(mock_core_class):
    ...
    completed_action = Action(
        name="Completed Task", slug="completed-task", status="completed"
    )
    next_action = Action(
        name="Next New Task", slug="next-new-task", status="in-progress"
    )

    # Configure the mock
    mock_core_instance = mock_core_class.return_value
    mock_core_instance.complete_current_and_start_next.return_value = (
        completed_action,
        next_action,
    )
