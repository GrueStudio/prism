...
def mock_core():
    ...
    # Create mock items
    phase = Phase(
        uuid="phase-uuid", name="Test Phase", slug="test-phase", status="pending"
    )
    milestone = Milestone(
        uuid="milestone-uuid",
        name="Test Milestone",
    ...
    core.navigator.get_item_by_path = Mock(
    ...
    core.navigator.get_item_path = Mock(
    ...
    def test_edit_with_uuid(self, runner, mock_core):
        ...
        with patch("prism.commands.crud.PrismCore", return_value=mock_core):
            result = runner.invoke(
                newcli, ["crud", "edit", "-u", "objective-uuid", "-s", "completed"]
            )
...
class TestCrudAdd:
    ...
    def test_add_phase(self, runner, mock_core):
        ...
        with patch("prism.commands.crud.PrismCore", return_value=mock_core):
            result = runner.invoke(
                newcli, ["crud", "add", "-t", "phase", "-n", "New Phase"]
            )
    ...
    def test_add_action_infers_parent(self, runner, mock_core):
        ...
        with patch("prism.commands.crud.PrismCore", return_value=mock_core):
            result = runner.invoke(
                newcli, ["crud", "add", "-t", "action", "-n", "New Action"]
            )
    ...
    def test_add_requires_parent_for_milestone(self, runner, mock_core):
        ...
        with patch("prism.commands.crud.PrismCore", return_value=mock_core):
            result = runner.invoke(
                newcli, ["crud", "add", "-t", "milestone", "-n", "New Milestone"]
            )
...
class TestTaskCommands:
    ...
    def test_task_next(self, runner, mock_core):
        ...
        mock_core.task_manager.complete_current_and_start_next = Mock(
            return_value=(completed, next_action)
        )
