...
def setup_test_project(tmp_path):
    ...
    with patch.object(Core, '_load_project_data', return_value=project_data):
        with patch.object(Core, '_save_project_data', return_value=None):
            core = Core(project_file=project_file)
            ...
            # without re-implementing a lot of the core logic here.

            # The best approach is to use the existing core logic to create the project,
...
def test_status_command_output(mock_core, tmp_path):
    ...
    mock_core.return_value.get_current_strategic_items.return_value = {
        "phase": None,
        "milestone": None,
        "objective": None
    }
    ...
    mock_core.return_value.get_status_summary.return_value = mock_summary
...
def test_status_command_with_phase_filter(mock_core):
    ...
    assert result.exit_code == 0
    mock_core.return_value.get_status_summary.assert_called_once_with(phase_path='test-phase-2', milestone_path=None)
...
def test_status_command_with_milestone_filter(mock_core):
    ...
    assert result.exit_code == 0
    mock_core.return_value.get_status_summary.assert_called_once_with(phase_path=None, milestone_path='test-phase-1/test-milestone-1')
