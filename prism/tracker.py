...
class Tracker:
    ...
    def update_item(self, path: str, name: Optional[str] = None, description: Optional[str] = None, due_date: Optional[str] = None, status: Optional[str] = None):
        ...
        if due_date is not None and isinstance(item_to_update, (Action, Deliverable)):
            try:
                ...
                raise ValueError(f"Invalid date format for due_date: {due_date}. Expected YYYY-MM-DD.")
    ...
    def delete_item(self, path: str):
        ...
        if parent_path:
            ...
            if target_list is not None:
                ...
                target_list[:] = [item for item in target_list if item.slug != item_slug_to_delete]
                
                if len(target_list) == original_len:
                    raise ValueError(f"Item with slug '{item_slug_to_delete}' not found under parent '{parent_path}'.")
            else:
                raise ValueError(f"Cannot delete child from parent of type {type(parent_item).__name__}")
        else: # Top-level phase deletion
            original_len = len(self.project_data.phases)
            self.project_data.phases[:] = [phase for phase in self.project_data.phases if phase.slug != item_slug_to_delete]
