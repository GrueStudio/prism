...
@click.option('--current-deliverable', 'current_deliverable_only', is_flag=True, help='Show only the current deliverable (containing the current action).')
    ...
    if json_output:
        ...
        if current_items["objective"]:
            result["current_strategic_focus"] = {
                "phase": {
                    "name": current_items["phase"].name if current_items["phase"] else None,
                    "status": current_items["phase"].status if current_items["phase"] else None
                } if current_items["phase"] else None,
                "milestone": {
                    "name": current_items["milestone"].name if current_items["milestone"] else None,
                    "status": current_items["milestone"].status if current_items["milestone"] else None
                } if current_items["milestone"] else None,
                "objective": {
                    "name": current_items["objective"].name if current_items["objective"] else None,
                    "status": current_items["objective"].status if current_items["objective"] else None
                } if current_items["objective"] else None
            }
    ...
    if not phase_path and not milestone_path:
        ...
        if current_items["objective"]:
            ...
            if current_items["phase"]:
                phase_status = f" ({current_items['phase'].status})" if current_items['phase'].status != 'pending' else ''
                click.echo(f"- Phase: {current_items['phase'].name}{phase_status}")
            if current_items["milestone"]:
                milestone_status = f" ({current_items['milestone'].status})" if current_items['milestone'].status != 'pending' else ''
                click.echo(f"- Milestone: {current_items['milestone'].name}{milestone_status}")
            if current_items["objective"]:
