...
def display_exec_tree(core, objective, current_action=None, current_deliverable_only=False):
...
def get_exec_tree_data(core, objective, current_action=None, current_deliverable_only=False):
...
@click.option('--current-deliverable', 'current_deliverable_only', is_flag=True, 
              help='Show only the current deliverable (containing the current action).')
@click.option('-j', '--json', 'json_output', is_flag=True, help='Output status in JSON format for AI agents.')
def status(current_deliverable_only, json_output):
    ...
    if current_objective:
        ...
        display_exec_tree(core, current_objective, current_action, current_deliverable_only)
        click.echo()
